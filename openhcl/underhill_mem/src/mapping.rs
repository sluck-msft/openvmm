...
pub enum GuestMemoryViewReadAccessType {
    ReadWrite,
    KernelExecute,
    UserExecute,
}
...
pub struct GuestMemoryView {
    ...
    pub read_access_type: GuestMemoryViewReadAccessType,
}
...
unsafe impl GuestMemoryAccess for GuestMemoryView {
    ...
    fn access_bitmap(&self) -> Option<guestmem::BitmapInfo> {
        ...
        if let Some(bitmaps) = self.memory_mapping.permission_bitmaps.as_ref() {
            match self.read_access_type {
                GuestMemoryViewReadAccessType::ReadWrite => {
                    return Some(guestmem::BitmapInfo {
                ...
                GuestMemoryViewReadAccessType::KernelExecute => {
                    return Some(guestmem::BitmapInfo {
                ...
                GuestMemoryViewReadAccessType::UserExecute => {
                    return Some(guestmem::BitmapInfo {
                        read_bitmap: NonNull::new(bitmaps.user_execute_bitmap.as_ptr().cast())
                            .unwrap(),
                        write_bitmap: NonNull::new(bitmaps.write_bitmap.as_ptr().cast()).unwrap(),
                        bit_offset: 0,
                    });
